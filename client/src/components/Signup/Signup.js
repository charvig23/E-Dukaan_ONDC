...
function Signup() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.post("http://localhost:4000/auth/register/admin", {
          name: name,
          email: email,
          password: password,
      });
      if (response.data.msg === "Admin registered successfully") {
  ...
  <div className="login-form-container" style={{height: '56%'}}>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className='haventsign' ><h5>Already have an account? <Link to= '/Login'>login</Link></h5></div>
      </form>
    </div>
    </div>
    </div>
